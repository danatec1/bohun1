...
- **기능**: 
  ...
  - 위탁병원 위치정보(GIS) → `/folium-map`
    - **NEW**: 광역지자체별 필터링 기능
    - 17개 시도 선택 가능 (서울, 부산, 대구, 인천, 광주, 대전, 울산, 세종, 경기, 강원, 충북, 충남, 전북, 전남, 경북, 경남, 제주)
...
- `GET /api/map/folium` - 지도 생성 API
  - 파라미터: `?region=서울` (선택사항)
  - 응답: `{ success, map_url, hospital_count, region }`
...
### v2.2 (2025-10-24)
- 🗺️ **Folium 지도 지역별 필터링 기능 추가**
  - 광역지자체별 선택 드롭다운 추가 (17개 시도)
  - 지역별 최적화된 중심 좌표 및 줌 레벨 설정
  - 지역별 병원 통계 실시간 표시
  - 종별 분류 및 색상 마커 유지
  - API 파라미터: `?region=서울` 형식 지원
